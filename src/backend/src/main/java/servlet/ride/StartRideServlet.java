...
public class StartRideServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            // Update ride state
            boolean success = RideService.getInstance().updateRideState(rideId, 0, "ENROUTE");
            ...
            if (success) {
                ...
                resp.getWriter().write(JSON.toJSONString(
                        CommonResult.success("ride started successfully", rideData)
                ));
            } else {
                resp.getWriter().write(JSON.toJSONString(
                        CommonResult.failed("failed to start ride")
                ));
            }
            ...
            e.printStackTrace();
            resp.getWriter().write(JSON.toJSONString(
                    CommonResult.failed("error starting ride: " + e.getMessage())
            ));
        }
    }
